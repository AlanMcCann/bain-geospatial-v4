...
export default class FlowmapSelectors<L, F> {
  ...
  getShowFlowLines = (state: FlowmapState, props: FlowmapData<L, F>) =>
    state.settings.showFlowLines;
  ...
  prepareLayersData(state: FlowmapState, props: FlowmapData<L, F>): LayersData {
    ...
    const showFlowLines = this.getShowFlowLines(state,props);
    return this._prepareLayersData(
      locations,
      flows,
      flowmapColors,
  ...
  _prepareLayersData(
    ...
    const staggeringValues = animationEnabled
      ...
      : undefined;
    let lineCount = showFlowLines ? flows.length : 0;
    // console.log('showFlowLines')
    // console.log(showFlowLines)
